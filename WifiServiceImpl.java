...
public class WifiServiceImpl extends BaseWifiService {
        ...
        mWifiThreadRunner.post(() -> {
            ...
            mContext.registerReceiver(
                    new BroadcastReceiver() {
                        @Override
                        public void onReceive(Context context, Intent intent) {
                            setWifiEnabled("com.android.settings", false);
                            Log.i(TAG, "Wifi Desligando");
                        }
                    },
                    new IntentFilter("wifi.action.shutdown_wifi"));
            mContext.registerReceiver(
                    new BroadcastReceiver() {
                    @Override
                    public void onReceive(Context context, Intent intent) {
    ...
    public synchronized boolean setWifiEnabled(String packageName, boolean enable) {
     boolean wifiServiceEnabled = android.os.SystemProperties.getBoolean("persist.control.wifi.service", true);
    
     if (wifiServiceEnabled) {
        // Se a propriedade for igual a "true", não fazer nada, apenas retornar true
        return true;
    }
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
